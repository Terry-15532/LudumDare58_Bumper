Shader "Custom/BasicAnimeWithVertMove"
{
    Properties
    {
        [Header(Basic)]
        ...
        [Header(Vert Settings)]
        [Space(10)]
        _VertOffset ("Vertex Wave Intensity", Vector) = (1,1,0,0)
        _VertSpeed ("Vertex Wave Speed", Vector) = (1,1,0,0)
...
//                "LightMode" = "UniversalGBuffer"
            ...
            {
                v2f o;
                // Vertex animation: offset along normal by sin(worldPos.x + worldPos.y + time)
                float3 worldPos = TransformObjectToWorld(v.vertex.xyz);
                float2 wave = float2(sin(0.3*(worldPos.x + worldPos.y) + _Time.y * _VertSpeed.x), sin(floor(worldPos.x/_VertSpeed.z)*_VertSpeed.z + _Time.y * _VertSpeed.y)) *
                _VertOffset;
                float3 animatedPos = v.vertex.xyz + float3(wave.x, 0, wave.y);
                o.position = TransformObjectToHClip(animatedPos);
                o.worldPos = TransformObjectToWorld(animatedPos);
                VertexPositionInputs positions = GetVertexPositionInputs(animatedPos);
            ...
            half3 frag(v2f i) : SV_Target
            {
                ...
                if (_LightEffect > 0){
                    ...
                    for (int idx = 0; idx < GetAdditionalLightsCount(); idx++){
                        ...
                        float NdotL = 0.9 * saturate(0.3 + 3 * abs(dot(TransformObjectToWorldNormal(i.normal), ldir)));
